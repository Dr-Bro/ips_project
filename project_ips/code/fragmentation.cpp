...
void low_level_fragmentation::VerticalSplitter(const Box& box, boxes_pair& vertical_splitter_pair)
{
	...
	width = width / 2;
	x_min = x_min + width;
	Box box2 = Box(x_min, y_min, width, height);
	vertical_splitter_pair = boxes_pair(box, box2);
}
...
void low_level_fragmentation::HorizontalSplitter(const Box& box, boxes_pair& horizontal_splitter_pair)
{
	...
	y_min = y_min + height;
	Box box2 = Box(x_min, y_min, width, height);
	horizontal_splitter_pair = boxes_pair(box, box2);
}
...
void low_level_fragmentation::GetNewBoxes(const Box& box, boxes_pair& new_pair_of_boxes)
{
	...
	current_box.GetWidhtHeight(x_width, y_height);
	if (x_width>y_height){
...
{
	// необходимо определить функцию
	double cur_box_diagonal = current_box.GetDiagonal();
	for (int i = 0; i < FindTreeDepth(); i++)
	{
